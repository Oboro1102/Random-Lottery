<template>
    ...
    <div class="control__item">
      <label>
        ...
        <input
          ...
          min="2"
          max="25"
          :disabled="drawData.length > 1"
        />
      </label>
      <button
        type="button"
        class="button"
        :disabled="drawData.length > 0"
        @click="setDrawData(drawAmount)"
      >
        載入資料
      </button>
    </div>
    <div class="control__item">
      <label>
        ...
        <input
          type="number"
          name="drawFrequency"
          v-model="drawFrequency"
          min="1"
          :max="drawData.length"
          :disabled="drawResult.length >= 1 || drawData.length < 1"
        />
      </label>
      <button
        type="button"
        class="button"
        :disabled="drawResult.length == drawFrequency || drawData.length < 1"
        @click="drawOnce(drawData, drawFrequency)"
      >
        抽出得獎者
      </button>
    </div>
  ...
  <main>
    <Pool :data="drawData" :sectionName="'抽籤池'">
...
export default defineComponent({
  ...
  setup() {
    ...
    function setDrawData(drawAmount: number) {
    ...
    function drawOnce(data: [], frequency: number) {
...
main {
  ...
  @include breakpoint-sm {
    height: calc(
      100% - #{$navbar-height}* 3 - #{$footer-height}* 2 - #{$grid-gap}* 2
    );
  }
}
